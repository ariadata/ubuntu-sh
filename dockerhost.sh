...
# Install PHPMyAdmin Locally
read -e -p $'Install PHPMyAdmin Locally [y/n]? : ' -i "n" if_install_pma
if [[ $if_install_pma =~ ^([Yy])$ ]]
then
	read -e -p $'Set PHPMyAdmin Internal Port : ' -i "9998" pma_internal_port
fi

# Install Nginx-Proxy-Manager
read -e -p $'Install Nginx-Proxy-Manager [y/n]? : ' -i "y" if_install_nginx_proxy_manager
...
# Install PHP-CLI
read -e -p $'Install PHP-CLI + composer [y/n]? : ' -i "y" if_install_php_cli_composer
if [[ $if_install_php_cli_composer =~ ^([Yy])$ ]]
then
	read -e -p $'Version [7.4|8.0]: ' -i "8.0" php_cli_version
	read -e -p $'Install Laravel Installer [y/n]: ' -i "y" if_install_laravel_installer
fi
...
if [[ $if_install_pma =~ ^([Yy])$ ]]
then
	docker run -d -p $pma_internal_port:80 --name phpmyadmin --restart=unless-stopped -e PMA_HOST=host.docker.internal -e MYSQL_USERNAME=sail -e MYSQL_ROOT_PASSWORD=password phpmyadmin
fi
...
then
	...
	then
		...
		# install with 7.4
		sudo apt --yes install php7.4-cli php7.4-dev php7.4-pgsql php7.4-sqlite3 php7.4-gd php7.4-curl php7.4-memcached php7.4-imap php7.4-mysql php7.4-mbstring php7.4-xml php7.4-zip php7.4-bcmath php7.4-soap php7.4-intl php7.4-readline php7.4-pcov php7.4-msgpack php7.4-igbinary php7.4-ldap php7.4-redis php7.4-swoole php7.4-apcu php7.4-xdebug
	fi

	sudo php -r "readfile('http://getcomposer.org/installer');" | sudo php -- --install-dir=/usr/bin/ --filename=composer
fi

## install laravel global + aliases
if [[ $if_install_laravel_installer =~ ^([Yy])$ ]]
then
